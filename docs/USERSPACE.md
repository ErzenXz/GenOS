...
GenOS 0.11 adds blocking scheduler states, parent-child ownership, and bounded inter-process messaging to the independently built ELF runtime. This document states exactly what the milestone proves and what it does not.
...
1. all instances query ABI version 4 and become eligible for timer scheduling;
...
The QEMU smoke test requires markers for sleep/block/wake, owned child wait/wake, message send/receive, asynchronous exit, kill, wait/reap, frame reclamation, fault containment, and the long-lived desktop. Recycled roots are visibly reused by later processes in the serial proof.
...
| 1 | `abi_version` | all zero | ABI version `4` |
...
- There is no blocking input, heap allocator, userspace filesystem API, or window API.
...
The next slice is copy-out plus structured userspace buffers, followed by blocking filesystem or device I/O built on the same saved-context wakeup model.
