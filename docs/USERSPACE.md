...
ABI 18 owns four socket objects per process and two fixed 128-byte queues per object. UDP connect enters `Established`; TCP-stream connect enters `Connecting` until its first bounded client transaction completes or fails. A send is admitted atomically only when the whole payload fits. A receive may copy a prefix and retains the unread suffix. Empty reads and saturated writes return `USER_ERROR_WOULD_BLOCK`. `socket_wait(handle, readiness, timeout)` either returns the observed readiness bits immediately or parks the process until readiness, a stale capability, or the bounded deadline; timeout returns `USER_ERROR_TIMED_OUT`.
...
ABI 18 binding is TCP-only and reserves ports below 1024. The process manager checks every live socket set before admitting a bind, so one local port has one owner system-wide. Listen fixes the backlog at one or two peers. Four passive transport slots exist globally, but one process incarnation may own at most two. Empty accept is non-blocking; an admitted peer makes the listener readable and accept-ready, and accept creates a separately revocable child capability in FIFO order. Allocation failure preserves the queued peer, handle-table failure rolls the child back, and close or owner cleanup makes the port immediately reusable.

The coordinator drives up to four concurrent checksum-validated passive SYN/SYN-ACK/ACK exchanges globally, capped at two for any one process incarnation. It queues each peer only if the same process incarnation, PID, typed handle, port, and backlog authority remain valid. Every accepted child retains its handshake's immutable MAC, address, ports, and initial sequences, including one bounded early data segment plus FIN acknowledged during handshake handoff. It becomes readable only after exact wire validation and writable while its bounded send queue has capacity; it remains excluded from the active client engine. Each accepted send completion uses its own nonzero request identity and requires the exact process slot, incarnation, task, PID, child handle, peer, byte count, and wire ACK.

The deterministic QEMU host connects two clients once the shell announces its listener. One verifies three bounded retransmission/backoff cycles after the network-test image drops three first responses. The other begins with reversed logical segments and then carries 1,024 total bytes through the same accepted capability. Ring 3 accepts while serving, uses `socket_wait` for accept/receive/drain/close progress, waits for peer half-close before write shutdown, waits for wire FIN completion before closing each capability, and closes a failed or reset peer so its slot can serve a replacement. The ordinary boot times out its optional listener safely when no peer exists. The shell also proves client socket authority, real UDP DNS, bounded TCP HTTP, RST/timeout/cancellation, listener denial and cleanup, and stale-handle rejection. Multi-segment outbound flight, multi-handle waits, broader fault coverage, and production TCP remain Stage 5.4.
...
`UserSystemInfo` is a `repr(C)` structure of twenty `u64` fields and reports the four-socket limit, 128-byte queue budget, and 40-byte status layout. ABI 18 preserves image-layout version `2`, the eight-page executable capacity, and the separate 24-byte `UserNetworkConfig`. `UserSocketStatus` reports protocol, lifecycle state, readiness bits, and queued send/receive bytes. The scheduler scans waiters from a rotating cursor and wakes at most two per pass, preventing an unbounded wake storm. `UserProcessStatus` reports an immutable instance ID, task ID, informational runtime PID, state, exit code, fault vector, and preemption count. These identity fields are observations, never authority. `UserDirectoryEntry` remains 96 bytes, `UserChannelMessage` 16 bytes, `UserFileStat` 32 bytes, and `UserInputEvent` 32 bytes. Their sizes, alignments, offsets, rights, limits, errors, masks, and codes are tested.
