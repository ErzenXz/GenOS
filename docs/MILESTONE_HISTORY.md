# Historical milestone record through GenOS 0.55

This preserves the earlier delivery record. ROADMAP.md at the repository root is the active plan; historical completion labels do not close its foundation gates.
...
**Status: in progress; bounded segmented recovery and interrupt regression gates complete; larger streams, scheduler wakeups, congestion control, and broad production gates remain**
...
Explicit boundary at completion: Stage 5.4D was one bounded request and response on one passive stream. Stage 5.4E adds bounded concurrency; arbitrary stream-length segmentation/reassembly, dynamic windows, congestion control, production loss recovery, and an application server framework remain open.

### Stage 5.4E — Concurrent passive handshakes and accepted-client service

**Status: complete**

Delivered:

- GenOS 0.50 two concurrent passive handshake slots and two concurrent accepted-stream slots keyed by exact peer tuple, with one shared bounded receive demultiplexer so concurrent passive operations never consume each other's frames;
- a two-cell pending-SYN queue with tuple deduplication, per-slot SYN-ACK retransmission and timeout, and per-tuple handshake cancellation that carries exact tuple identity through failure reporting;
- bounded early-segment admission: a handshake slot accepts and acknowledges at most one 128-byte data segment plus FIN arriving between the peer's final handshake ACK and stream attachment, then seeds the accepted stream with those exact bytes instead of dropping them;
- rotating fair service across stream slots, with a slot holding unread Ring 3 bytes re-acknowledging its current sequence without admitting new data, so one slow reader cannot lose bytes or starve its sibling stream;
- per-slot coordinator records binding each handshake to its exact listener capability and each stream to its claimed capability, in-flight send identity, and close progress, with per-stream stale-capability, stale-send, and unclaimed cancellation;
- a Ring 3 bounded service window that accepts and serves multiple clients concurrently, closes failed or reset peers so their slots serve replacement clients, and answers each exact `GENOS_PING*` request with its matching `GENOS_PONG*` response through that stream's own capability;
- an explicit `USER_SOCKET_PASSIVE_LISTEN_READY` marker that starts the deterministic host clients only after the listener really exists, plus `USER_SOCKET_PASSIVE_CONCURRENT_READY streams=2` required by the network gate;
- a QEMU proof in which two real host clients complete request, response, half-close, and EOF through one listener window, while the no-host boot still completes without any passive success marker.

Acceptance criteria:

- [x] Two passive handshakes and two accepted streams can exist at once, each bound to its exact tuple, listener, and process authority.
- [x] One shared receive pump routes every frame to exactly one owning slot, and a connection beyond the fixed slot budget is refused without ending service.
- [x] A bounded early segment between the final handshake ACK and stream attachment is acknowledged exactly once and delivered to Ring 3 rather than silently discarded.
- [x] Per-stream reset, idle timeout, stale capability, and stale send fail closed without disturbing the sibling stream, and the Ring 3 window accepts a replacement client.
- [x] Two real host clients complete full request/response/close cycles through one listener window on modern-only VirtIO 1.x.

Explicit boundary: concurrency is fixed at two handshake slots and two stream slots, each stream still carries one bounded request and response, and Ring 3 discovers progress by polling with bounded sleeps rather than scheduler readiness wakeups. The reference host network (QEMU slirp) may serialize forwarded connections, so wire-level handshake overlap is exercised by the slot machinery and unit tests rather than guaranteed by the host. Arbitrary segmentation/reassembly, dynamic windows, congestion control, production loss recovery, and interrupt-driven completion remain open below.

### Stage 5.4F — Bounded segmented-stream recovery

**Status: complete**

Delivered:

- GenOS 0.51 keeps an accepted capability established across multiple Ring 3 receive/send cycles instead of forcing close after the first response;
- one fixed 128-byte delivery buffer plus one fixed 128-byte deferred segment, cumulative acknowledgments, duplicate suppression, in-order promotion after a gap closes, and a receive window derived from actual free bounded storage;
- per-stream RTT samples, a bounded adaptive retransmission deadline, exponential retry backoff, and the existing fixed retry ceiling;
- a test-only kernel feature used only by `cargo xtask test-network` that deterministically drops one response and reverses two logical 128-byte segments without placing production trigger payloads in normal images;
- a QEMU proof that Ring 3 receives both reordered chunks in order, answers both through the same accepted capability, and recovers the deliberately dropped response before orderly close.

Acceptance criteria:

- [x] One accepted capability completes more than one request/response cycle without reopening or losing exact peer authority.
- [x] A fixed out-of-order slot never exceeds the documented 256-byte receive budget and releases bytes to Ring 3 only in sequence order.
- [x] Deterministic loss causes a bounded retransmission, and deterministic reordering preserves exact payload bytes and close behavior.
- [x] The fault-injection path is absent from production images and enabled explicitly only for the network acceptance image.

Explicit boundary: this is bounded recovery, not arbitrary TCP. Each segment is at most 128 bytes, only one deferred segment and one outbound send may exist per stream, concurrency remains two streams, and congestion control, SACK, large windows, and general Internet interoperability remain open.

### Stage 5.4G — Interrupt-driven VirtIO and regression budgets

**Status: complete**

Delivered:

- GenOS 0.52 configures PCI MSI-X entry 0 at vector 48 and assigns both modern RX/TX virtqueues to that shared vector;
- the interrupt handler records readiness and acknowledges the local APIC while descriptor validation, ownership transfer, copying, and recycling remain in coordinator context;
- normal TX waits for an interrupt epoch and normal RX consults the used ring only after interrupt readiness, with a fixed 32,768-call recovery interval and the original hard timeout retained for lost interrupts;
- monotonic counters for interrupts, interrupt-driven RX/TX completions, recovery polls/completions, frames, and queue notifications;
- a live QEMU regression gate covering payload throughput, elapsed ticks, maximum ACK latency, retransmission count, reordered segments, peak buffered bytes, interrupt/recovery ratio, recovery work, notification work, and the fixed eight-entry queue capacity.

Acceptance criteria:

- [x] Modern VirtIO RX and TX complete normally through MSI-X, and descriptor ownership is never transferred in interrupt context.
- [x] Recovery polling is bounded, observable, and a minority path rather than the normal completion mechanism.
- [x] Throughput, elapsed time, ACK latency, recovery, CPU-work proxies, and queue occupancy have machine-checked QEMU budgets.
- [x] The modern-only network gate requires MSI-X, deterministic fault recovery, ordered Ring 3 delivery, and the regression-budget marker.

Explicit boundary: GenOS uses one shared MSI-X vector and one queue pair. Per-queue vectors, queue batching, multiple queue pairs, real-device interrupt routing, and negotiated offloads remain future performance work.

### Stage 5.4H — Long-lived bounded streams and congestion response

**Status: complete**

Delivered:

- GenOS 0.53 carries at least 1,024 payload bytes through one accepted capability while retaining the fixed 256-byte receive-storage ceiling and recycling that window only after Ring 3 consumes ordered data;
- per-stream peer-window observation, cumulative byte accounting, a 128-byte MSS, a 256-byte initial congestion window, slow start followed by additive increase, a 1,024-byte hard congestion-window ceiling, and multiplicative decrease to one MSS on retransmission timeout;
- a deterministic sustained-loss workload that drops the first transmission of three consecutive responses, proves three bounded backoffs and recoveries, and retains exact response bytes across the same connection;
- release-gated stream-byte, retransmission, reordering, congestion-event, congestion-window, ACK-latency, buffer, interrupt, recovery, frame, and notification budgets.

Acceptance criteria:

- [x] A single accepted stream transfers more than the fixed 256-byte receive window without increasing its bounded resident receive storage.
- [x] Sustained deterministic loss causes repeated multiplicative decrease and successful bounded recovery rather than an unbounded retry loop.
- [x] Congestion state remains per stream, is bounded between one MSS and 1,024 bytes, and is included in the QEMU regression gate.

Explicit boundary: Ring 3 still submits and receives 128-byte chunks, one response segment is outstanding per stream, and the congestion controller is a bounded stop-and-wait AIMD foundation rather than a full multi-segment Reno/CUBIC implementation. SACK, fast retransmit, arbitrary windows, and broad Internet interoperability remain open.

### Stage 5.4I — Scheduler-backed socket readiness and fair owner budgets

**Status: complete**

Delivered:

- GenOS 0.54 and ABI 18 `socket_wait`, which validates one exact process-owned socket capability plus a readiness mask and bounded deadline, returns immediately when already ready, or parks the process in `Waiting` without a Ring 3 sleep loop;
- scheduler-side readiness and timeout wakeups with a rotating process cursor and a hard budget of two socket wakes per coordinator pass;
- four global passive handshake and stream slots with a hard two-slot budget per process incarnation, preserving two-client service for one listener while reserving capacity for a second process owner;
- Ring 3 passive accept, receive, drain, and close service driven through readiness waits, with live block, wake, immediate-ready, and timeout markers required by QEMU.

Acceptance criteria:

- [x] Ring 3 can wait for accept, receive, send-drain, close, or error readiness without polling through `sleep`.
- [x] A stale or forged socket grants no wait authority, and every wait has a maximum 10,000-tick deadline.
- [x] Wake scanning and passive ownership are bounded and rotate fairly across process slots; one process incarnation cannot consume more than two of the four passive transport slots.

Explicit boundary: the current ABI waits on one socket at a time, not an epoll-style set. The live QEMU service uses one Ring 3 listener, while cross-process cursor and ownership rules are enforced structurally and by host tests; multi-process application-server integration remains future work.

### Remaining Stage 5.4 work

GenOS 0.55 adds the Stage 5.4J host packet matrix: duplicate delivery, multi-gap deferred data and FIN, reset sequence validation and bounded challenge ACKs, forged ACK/window rejection, zero-window persistence and small-window segmentation, Karn RTT sampling, slow-reader retransmission progress, sequence wrap, cancellation isolation, slot exhaustion/reuse, and checksum/truncation rejection. These tests execute the production packet/state-machine code with a memory frame device. Native QEMU retains the sustained-loss, reordering, concurrent Ring 3, and modern VirtIO gates. This completes a bounded host matrix, not the physical-device or full production TCP gate below.

- multi-segment outbound flight, larger dynamic send/receive windows, more than one out-of-order slot, selective acknowledgments, fast retransmit, and production loss recovery;
- readiness sets spanning several handles, live multi-process application servers, and more than two clients on one listener;
- deterministic duplication, delay, zero-window, reset, slow-reader, cancellation, and resource-exhaustion wire tests beyond the bounded loss/reorder proof;
- per-queue MSI-X, measured batching, real-device interrupt routing, multiple queue pairs, and optional offload negotiation with fallback tests.
...
- [x] A listening service survives concurrent clients, cancellation, peer resets, and slow readers without sharing raw port authority (four global handshake/stream slots, capped at two for one process owner).
- [x] TCP transfers remain correct under deterministic loss and reordering and respect bounded memory budgets (one retransmission and one deferred 128-byte segment).
- [x] VirtIO RX/TX normally completes through MSI-X; polling remains only a bounded, measured recovery mechanism.
- [x] Throughput, latency, packet-loss recovery, CPU-work proxies, and queue occupancy have reproducible QEMU regression budgets.
- [x] General streams exceed the fixed 256-byte receive window safely and apply congestion control under sustained loss (1,024 total bytes, fixed 256 resident bytes, bounded stop-and-wait AIMD).
- [x] Scheduler readiness wakes Ring 3 without bounded sleep polling, with fair cross-process listener and stream budgets (one-handle ABI wait, two wakes per pass, two of four passive slots per owner).
- [ ] The broader duplication, delay, zero-window, reset, slow-reader, cancellation, exhaustion, and physical-device matrix passes.

## Stage 5.5 — IPv6 dual stack

**Status: in progress; required before production networking**

Goal: make IPv6 a first-class path while preserving explicitly tested IPv4 compatibility.

Stage 5.5A is delivered in GenOS 0.55: bounded IPv6 and padding-only extension validation, mandatory transport checksum helpers, scheduler-driven link-local/global DAD, router solicitations, validated router advertisements and /64 SLAAC, expiring route/address lifetimes, neighbor replies, and an exact ICMPv6 echo over VirtIO. Both native QEMU network gates require the SLAAC/DAD and echo markers. Initialization still depends on the current IPv4 reference interface; IPv6 sockets, DNS, IPv6-only boot, PMTU, and broader ND remain open. See [the IPv6 contract](IPV6.md).
...
- [x] GenOS configures a usable IPv6 address and default route without a hard-coded guest address (bounded SLAAC/ICMPv6 reference path).
...
Stage 7A is delivered in GenOS 0.55: `cargo xtask new-app PATH` exports an independent offline Rust workspace containing the ABI/runtime, a guarded-layout linker script, and a minimal application. `cargo xtask test-sdk` builds outside the repository, loads the ELF through the production mapper, executes it in Ring 3, checks output/exit, and reclaims its address space. ABI 18 mismatch exits explicitly; general named launch, package distribution, compositor/UI, and a stable SDK release policy remain open. See [the SDK contract](SDK.md).
...
- [x] An application can be built outside the main repository using the SDK.
