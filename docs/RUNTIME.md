...
GenOS 0.54 separates operating-system runtime work from presentation, removes duplicated userspace lifecycle state, gives every process one typed handle authority table, binds every deferred service operation to an exact request identity, and makes the shell a fail-closed session supervisor. `RuntimeCoordinator` owns the kernel task registry, `ProcessManager`, mounted VFS, persistent snapshot coordinator, scheduler advancement, the pending lifecycle and VFS queues, one bounded asynchronous UDP-or-TCP client transport slot, four passive TCP handshake slots, and four passive stream slots keyed by exact peer tuple. At most two passive slots belong to one process incarnation. Accepted streams may carry more than their resident receive window while retaining at most one 128-byte delivery segment and one 128-byte deferred segment. `ProcessManager` owns ABI 18 listener, accepted-child, and one-handle readiness-wait authority; it wakes at most two socket waiters per pass using a rotating process cursor. Passive receive, send completion, wait wakeup, and close are revalidated against the exact live process and socket identity. The active product surface is the serial terminal; the old `DisplayManager` code is deferred until the later graphical rebuild.
...
| pending lifecycle, VFS, typed UDP/TCP client requests, passive handshake, and accepted-stream send | `RuntimeCoordinator` | no direct presentation access |
...
The current coordinator has one pending VFS request, one pending lifecycle request, and one pending typed socket client request because the scheduler runs at most one userspace slice per tick, plus four passive handshake slots and four passive stream slots keyed by exact peer tuple. One process incarnation may consume at most two passive slots. During headless boot, `run_headless_boot_probe` advances the actual Ring 3 shell until VFS and child-lifecycle requests with distinct IDs have completed. It also drives UDP/TCP clients, concurrent passive handshakes, accepted receive delivery, response ACK, congestion backoff, and FIN progress outside the syscall path. Client completion revalidates the process, protocol, capability, request ID, destination, and admitted bytes. Each passive handshake completion revalidates its listener process slot, incarnation, PID, typed handle, and port. Each accepted send completion additionally binds the task ID, child handle, immutable peer state, request ID, and byte count; receive, readiness wake, and close revalidate the same child authority before mutation. Closing a listener cancels only its own pending handshakes and unclaimed streams, while closing, exiting, faulting, or killing an accepted owner resets that stream without disturbing a sibling. The boot suite still proves canceled and replayed work cannot mutate external state. Multi-handle readiness sets and multi-request client queues remain future runtime work.

## Handle rights

Console, file, endpoint-send, endpoint-receive, lifecycle, process, and socket handles all register in one bounded `HandleTable` owned by the caller. The table records the exact opaque ABI value, type, and rights. File operations request `READ`, `WRITE`, or `MANAGE` at lookup; the other families request their typed use/control right. Subsystem arrays hold only object metadata and cannot grant authority on their own.
...
A later application stage can replace `DisplayManager` with a window server without changing process, capability, VFS, or socket-service ownership. The display-disabled boot proof now covers scheduling, VFS, lifecycle, UDP, and TCP client completion, exact asynchronous identities, cancellation, snapshots, and unified handle authority without coupling those services back to rendering.
