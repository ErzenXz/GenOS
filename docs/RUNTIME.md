...
The current coordinator has one pending VFS request, one pending lifecycle request, one pending typed socket client request, and one passive TCP handshake-or-stream because the scheduler runs at most one userspace slice per tick. During headless boot, `run_headless_boot_probe` advances the actual Ring 3 shell until VFS and child-lifecycle requests with distinct IDs have completed. It also drives UDP/TCP clients, passive handshake, accepted receive delivery, response ACK, and FIN progress outside the syscall path. Client completion revalidates the process, protocol, capability, request ID, destination, and admitted bytes. Passive handshake completion revalidates the listener process slot, incarnation, PID, typed handle, and port. Accepted send completion additionally binds the task ID, child handle, immutable peer state, request ID, and byte count; receive and close revalidate the same child authority before mutation. Closing a listener cancels an unclaimed stream, while closing, exiting, faulting, or killing the accepted owner resets active wire state and prevents later delivery. The boot suite still proves canceled and replayed service work cannot mutate external state. Long-lived streams, fair multi-socket service, simultaneous passive handshakes, and multi-request queues remain future runtime work.
